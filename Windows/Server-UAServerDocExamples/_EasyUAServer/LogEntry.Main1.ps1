...
using namespace OpcLabs.BaseLib.Instrumentation
...
$EasyUAServerOnLogEntry = {
    param ($sender, $e)
    Write-Host $e
}

# Hook static events.
$logEntryEventHandler = [RunspacedDelegateFactory]::NewRunspacedDelegate([LogEntryEventHandler]$EasyUAServerOnLogEntry)
[EasyUAServer]::add_LogEntry($logEntryEventHandler)
...
[EasyUAServer]::remove_LogEntry($logEntryEventHandler)
