...
$server.Add([UADataVariableExtension]::WriteValueAction((New-Object UADataVariable("WriteToThisVariable")), 
    [RunspacedDelegateFactory]::NewRunspacedDelegate([System.Action[byte[]]]{ 
        param($value)
        Write-Host "Value written: $(if($value -eq $null) { [String]::Empty } else { [BitConverter]::ToString($value) })"
    })))
